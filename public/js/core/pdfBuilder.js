 ...
 */
export async function buildPdf({ header, results, metric, allEntries, brand, logoDataUrl, countyFipsCode }) {
  ...
  // Form Number — lower-right footer, same row as the "Generated by..."
  // note on the left (see core/formNumber.js's top comment for what this
  // identifier is and how it's assigned). Omitted entirely for a plot
  // that doesn't have one yet (offline on its first export — see
  // ui/formNumberAssign.js) rather than printing a blank/placeholder
  // label.
  const formNumber = assembleFormNumber(header, countyFipsCode || null);
  if (formNumber) {
    doc.text(`Form Number: ${formNumber}`, MARGIN + tableWidth, PAGE_HEIGHT - MARGIN, { align: "right" });
  }
  doc.setTextColor(0, 0, 0);
 ...
 * @param {string|null} [countyFipsCode] see exportFilename() in xlsxBuilder.js
 * @returns {string}
 */
export function pdfFilename(header, countyFipsCode) {
  return exportFilename(header, countyFipsCode).replace(/\.xlsx$/, "_Results.pdf");
}
