...
export async function buildPdf(args) {
  ...
  renderReport(doc, args);
  return doc.output("blob");
}

/**
 * The whole report's drawing, split out from buildPdf() above so the
 * layout can be exercised against the mocked jsPDF test doubles. Uses
 * only the primitive subset those mocks also implement (see e2e_pdf_*
 * test doubles): setFont/setFontSize/set*Color/setLineWidth,
 ...
 * }} args
 */
export function renderReport(doc, { header, results, metric, allEntries, brand, logoDataUrl, includePlotDetails = false }) {
  ...
  doc.text(`Generated by Republic Regional Seed Network on ${new Date().toLocaleString()}`, MARGIN, footerY);
  ...
  if (header.formId) {
    doc.text(`Form ID: ${header.formId}`, MARGIN + tableWidth, footerY, { align: "right" });
  }
  doc.setTextColor(0, 0, 0);
}
