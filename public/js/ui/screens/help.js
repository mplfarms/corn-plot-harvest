...
export function render(container) {
  ...
  const sections = [
    ...
    helpSection("Starting a New Plot", [
      ...
      p("The capture also fills in State, County, and the nearest City for you when those fields are still blank — the County and City always come from the app's own lists (real towns, never townships), so the ZIP auto-fill keeps working off the filled City. Anything you've already typed or picked is never overwritten (with one exception: a brand-new plot's untouched default State gets corrected to where you're actually standing). No signal, or no confident match? Those fields simply wait for manual entry, same as Soil Type below."),
