...
export function render(container) {
  ...
  const sections = [
    ...
    helpSection("Starting a New Plot", [
      ...
      p("On a desk computer (no touch screen), the button warns you first: a computer estimates its location from the internet connection, not GPS, and can be miles off. For an exact spot from a desk, use “Pick Location on Map” instead — it opens a satellite map (free government imagery with highways, rivers, county lines, and town names marked so it's easy to orient; needs an internet connection) where you zoom to the field and tap it. The pin can be dragged to fine-tune, and confirming fills the coordinates and runs the same County/City/Zip and Soil Type lookups as a device capture. A map-picked location counts as manually set, so the device button will ask before ever overriding it."),
      p("The capture also fills in State, County, and City for you when those fields are still blank. For City, the app searches for every incorporated town within 10 miles of where you're standing (widening to 15 miles in open country), auto-fills the NEAREST one, and shows the full nearest-first list — with distances — right under the City field so you can tap a different town if the nearest isn't the right mailing address; the Zip follows whichever town is picked. County and City always come from the app's own lists (real towns, never townships). Anything you've already typed or picked is never overwritten (with one exception: a brand-new plot's untouched default State gets corrected to where you're actually standing). No signal, or no town found? Those fields simply wait for manual entry, same as Soil Type below."),
